...
function updateUI(row, column, color, moveNumber) {
    ...
    moveLbl.textContent =`Volgende zet: (${moveNumber + 1})`;
}
...
function updatePlayerTurn() {
    ...
    if (gameState.gameOver) {
        playerTurnElement.textContent = `Game Over! ${gameState.currentTurn.toUpperCase()} wins!`;
    } else {
        playerTurnElement.textContent = `Player Turn: ${gameState.currentTurn.toUpperCase()}`;
    }
    playerTurnElement.classList.remove('red', 'black'); // Remove previous player class
    playerTurnElement.classList.add(gameState.currentTurn); // Add current player class
}
...
// // Function to parse and apply the game state with gravity
// function applyGameStateFromInput(input) {
//     // Reset the board
//     gameState.board = Array.from({ length: 6 }, () => Array(7).fill(null));
//     gameState.moveHistory = [];

//     // Match all cell() patterns in the input
//     // const cellRegex = /cell\((\d),(\d),(\d)\)/g;
//     // const cellRegex = /(?:cell|next)\((\d),(\d),(\d)\)/g;
//     cellRegex = /(?:cell|next|)\((\d),(\d),(\d)\)/g;
//     let match;

//     const columnPieces = Array(7).fill().map(() => []); // Store pieces by column

//     while ((match = cellRegex.exec(input)) !== null) {
//         const col = parseInt(match[1], 10) - 1; // Convert to 0-based index
//         const player = parseInt(match[3], 10);

//         // Add player piece to the corresponding column array
//         if (player === 1) {
//             columnPieces[col].push('red');
//         } else if (player === 2) {
//             columnPieces[col].push('black');
//         } // player === 0 (empty) is ignored
//     }

//     // Apply gravity and update the board
//     columnPieces.forEach((pieces, col) => {
//         let row = 5; // Start from the bottom row
//         pieces.reverse().forEach((piece) => {
//             gameState.board[row][col] = piece; // Place the piece
//             gameState.moveHistory.push({ row, column: col, color: piece }); // Add to move history
//             row--; // Move up to the next available row
//         });
//     });
// }
...
function updateBoardUI() {
    ...
    // Automatically hide all move numbers
    // const moveNumberElements = document.querySelectorAll('.piece span'); // Select all move number spans
    // moveNumberElements.forEach(el => el.classList.add('hidden')); // Add the hidden class to hide them

    // Update the text scheme
    updateTextScheme();
}

// Function to simulate a game 
document.getElementById('simulateBtn').addEventListener('click', () => {
    simulateGame(threatOrRandomStrategy,threatBasedStrategy);
    // simulateGame(threatOrRandomStrategy,mimicOpponentStrategy);
    // simulateGame(randomStrategy,threatBasedStrategy);
    // simulateGame(mimicOpponentStrategy,threatOrRandomStrategy);
});

async function simulateGame(strategyWhite, strategyBlack) {
    resetGame(); // Step 1: Initialize the game
    let currentPlayer = 'red'; // Step 2: White player starts

    while (!gameState.gameOver) {
        // Step 3: Call doMove with the appropriate strategy
        const strategy = currentPlayer === 'red' ? strategyWhite : strategyBlack;
        const chosenColumn = doMove(strategy);

        // // Step 4: Add the move to the gameNote textarea
        // const gameNote = document.getElementById('gameNote');
        // gameNote.value += `Player ${currentPlayer} chose column ${chosenColumn + 1}\n`;

        // Step 5: Effectuate the move and update the game state
        placePieceInColumn(chosenColumn);

        // // Step 6: Append the current game state to the cumulative file
        // appendCurrentGameState();

        // // Step 7: Refresh the board on screen
        // updateBoardUI();

        // Step 8: Wait for 1 second
        await delay(100);

        // // Step 9: Switch player turn
        // currentPlayer = currentPlayer === 'red' ? 'black' : 'red';

        // // Step 10: Check for a winning situation
        // const winner = detectWinner();
        // if (winner) {
        //     gameState.gameOver = true;

        //     // Step 11: Export the final cumulative game state when the game ends
        //     saveCumulativeGameStates();
